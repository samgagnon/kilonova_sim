...
def convert_flux(flux, wavelength):
    ...
    flux_density = 1e-23 * flux * (wavelength ** 2) / c
    mAB = 8.9 - 2.5 * np.log10(flux_density / 3631)
    return mAB
...
def main(time, instrument, bns):
    ...
    MMT_pointings, CSS_pointings, GOTO_pointings = get_inst_pointings(pointings)
    mmt_time, mmt_band, mmt_depth = get_important_things('MMT')
    goto_time, goto_band, goto_depth = get_important_things('GOTO-4')
    css_time, css_band, css_depth = get_important_things('CSS')
    mmt = {}
    ...
    if detection is None:
        ...
        for wave in wavecollection:
            for waveform in wave:
                if r1 < waveform[0] < r2:
                    mAB = convert_flux(waveform[i], waveform[0])
                    print(mAB, depth)
                    if mAB > depth:
