...
elif (subdir1 == 'esg100m') or (subdir1 == 'esgm'):
    labels = ['Original', 'Malmquist-corrected']
    max_arr = [-104.69960725284007, -104.69960725284007]

elif (subdir1 == 'esgo'):
    labels = ['Original', 'GW-only incl.']
    max_arr = [-104.69960725284007, -280.52443031806354]
...
def S_filter(H0, H0min, S, m, smin, smax, truemin):
    ...
    if S == truemin[0]:
        ...
        Sf = ((S-smin)/(smax-smin)*f)*(smax-smin)+smin
        if Sf < truemin[0]:
            return truemin
        else:
            return Sf
...
for i in [0, 1]:
    ...
    if i == 0:
        ...
        dtl = filter_dataset(dtl, -1*lr2[0], H0_max, max_arr[0], newmin)
        datasets2 += [dtl]

        # torch.save(dtl, 'data archive/esg25100-Sm.dataset.pt')
    ...
    # if i == 1:
    #     print(len(dtl))
    #     dtl = clean_m_dataset(dtl, min_val[0], 69.5, 85)
    #     print(len(dtl))
    # datasets2 += [dtl]
